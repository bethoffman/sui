...
The frontmatter at the top of MDX pages provides context for the current topic. The only frontmatter entry required at the time of writing is `title`. In addition to the standard frontmatter entries available, the following custom items are useful for certain topic types.

### beta

Including the `beta` entry applies a box to the top of the topic that informs the reader of the beta status of the feature or service described. Possible values include `true` for a standard box, or a list of environments the feature or service is available in (`devnet`, `testnet`, `mainnet`) to include that information in the note.
